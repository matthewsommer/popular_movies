...
/**
 * User preferences class to wrap around SortPreference Created by matt on 10/17/17.
 */
...
public class SortPreference {
  ...
  public static String getText(Activity activity) {
    ...
    if (sharedPref.getInt(activity.getResources().getString(R.string.pref_sort_order_key), 0)
        == SORT.POPULARITY.getCode()) {
      return activity.getResources().getString(R.string.recently_popular);
    } else if (sharedPref.getInt(activity.getResources().getString(R.string.pref_sort_order_key), 0)
        == SORT.RATING.getCode()) {
